...
cmd_handler(cmd_x) {
	if(args == NULL) 
	{ 
		puts("Command format: \"x NUM EXPR\"");
		return 0; 
	}
	char *check_legal=args;
	bool have_tab=false;
	while(check_legal!=NULL)
	{
		if(*check_legal==' ')
		{
			have_tab=true;
	...
	int i=0;
	char *pointer=args;
	for(;pointer!=check_legal;pointer++,i++)
	{
		if(*pointer<'0'||*pointer>'9')
		{
			printf("the middle number should only have symbol between 0 and 9\n");
			puts("Command format: \"x NUM EXPR\"");
			return 0;
		}
		num[i]=*pointer;
	...
	bool success;
	uint32_t addr=expr(pointer+1, &success);
	if(!success)
	{
		printf("invalid expression: '%s'\n", args);
		return 0;
	}
	uint32_t count=atoi(num);
	if(count<=0)
	{
		printf("check memory at least for one\n");
		return 0;
	}
	printf("0x%x(%d): ", addr, count);
	for(int i=0;i<count;i++)
	{
		if(addr>=128*1024*1024||addr<0)
		{
			printf("the address must be a valid number between 0x0(0) and 0x%x(%d)\n", 128*1024*1024, 128*1024*1024);
			return 0;
		}
		uint32_t result=vaddr_read((vaddr_t)addr, 0, 4);
		printf(" 0x%x", result);
		addr+=4;
	}
	printf("\n");
